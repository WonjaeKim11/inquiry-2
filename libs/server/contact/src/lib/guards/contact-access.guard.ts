...
/** 필요한 최소 역할 메타데이터 키 */
export const CONTACT_MIN_ROLE_KEY = 'contact_min_role';

/**
 * 연락처 접근 최소 역할을 지정하는 데코레이터.
 * ContactAccessGuard와 함께 사용하여 컨트롤러/핸들러에 필요한 최소 역할을 지정한다.
 * 지정된 역할 이상의 권한을 가진 멤버만 접근 가능하다.
 *
 * @example @ContactMinRole('ADMIN')  // OWNER 또는 ADMIN만 접근
 * @example @ContactMinRole('MEMBER') // 모든 멤버 접근 가능
 */
export const ContactMinRole = (minRole: 'OWNER' | 'ADMIN' | 'MEMBER') =>
  SetMetadata(CONTACT_MIN_ROLE_KEY, minRole);
 ...
 * 5. @ContactMinRole() 데코레이터에 지정된 최소 역할과 비교한다.
 */
@Injectable()
export class ContactAccessGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const minRole = this.reflector.getAllAndOverride<string | undefined>(
      CONTACT_MIN_ROLE_KEY,
      [context.getHandler(), context.getClass()]
    );

    if (minRole) {
      const userLevel = ContactAccessGuard.ROLE_HIERARCHY[membership.role] ?? 0;
      const requiredLevel = ContactAccessGuard.ROLE_HIERARCHY[minRole] ?? 0;
