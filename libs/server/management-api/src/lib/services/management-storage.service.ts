...
/**
 * Management API Storage 서비스.
 * Management API를 통한 파일 업로드를 처리한다.
 * STORAGE_PATH 환경변수로 저장 경로를 설정할 수 있다 (기본값: ./uploads).
 */
@Injectable()
export class ManagementStorageService {
  private readonly logger = new Logger(ManagementStorageService.name);
   ...
   * management 전용 디렉토리에 환경별로 분리 저장한다.
  ...
  async uploadFile(
    ...
    const dirPath = path.join(this.storagePath, 'management', environmentId);
    ...
    this.logger.log(`Management 파일 저장 완료: ${filePath}`);

    return {
      url: `/storage/management/${environmentId}/${storedName}`,
    };
  }
}
